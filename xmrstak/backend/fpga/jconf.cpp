...
namespace xmrstak
{
	namespace fpga
	{
		...
		*/
		enum configEnum { aFpgaThreadsConf };
		...
		configVal oConfigValues[] = {
			{ aFpgaThreadsConf, "fpga_threads_conf", kNullType }
		};
		...
		{
			if (!prv->configValues[aFpgaThreadsConf]->IsArray())
				return false;

			if (id >= prv->configValues[aFpgaThreadsConf]->Size())
				return false;

			const Value& oThdConf = prv->configValues[aFpgaThreadsConf]->GetArray()[id];

			if (!oThdConf.IsObject())
				return false;

			const Value *mode, *no_prefetch, *aff;
			mode = GetObjectMember(oThdConf, "com_port");
			...
			cfg.iComPort = (int)mode->GetInt64();
		...
		{
			if (prv->configValues[aFpgaThreadsConf]->IsArray())
				return prv->configValues[aFpgaThreadsConf]->Size();
			else
				return 0;
		}
		...
		bool jconf::parse_config(const char* sFilename)
		{
			...
			for (size_t i = 0; i < GetThreadCount(); i++)
			{
				...
				{
					printer::inst()->print_msg(L0, "FPGA: Thread %llu has invalid config.", int_port(i));
					return false;
				}
			}
